...
훅을 이해하는 가장 빠른 길은 "Claude Code가 한 턴(turn)을 처리하는 동안 언제 끼어드는가"를 그림으로 보는 것입니다. 아래는 사용자가 한 번 입력했을 때 다섯 종류의 훅이 어느 지점에서 발동하는지를 나타낸 흐름입니다.
...
> [!NOTE]
> `SessionStart`만 "한 턴"의 바깥, 즉 세션이 열리는 시점에 단 한 번 돕니다. 나머지 네 종류는 사용자 입력 → 도구 사용 → 응답으로 이어지는 매 턴의 흐름 안에서 반복적으로 발동합니다.
...
| 🟡 `UserPromptSubmit` | 사용자가 입력할 때마다 | 모호한 프롬프트 감지 | 매 입력 |
